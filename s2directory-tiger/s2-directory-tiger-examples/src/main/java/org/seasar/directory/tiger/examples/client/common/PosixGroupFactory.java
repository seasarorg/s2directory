...
public class PosixGroupFactory extends DirectoryDtoFactory {
	 ...
	 * @return グループインスタンス
	 */
	public PosixGroup getGroup(String name, List<PosixGroup> users) {
		PosixGroup group = new PosixGroup();
		group.setDn("cn=" + name + ",ou=Groups," + property.getBaseDn());
		group.setCn(name);
		group.setGidNumber("1001");
		StringBuffer buffer = new StringBuffer();
		for (Iterator<PosixGroup> iter = users.iterator(); iter.hasNext();) {
	 ...
	 * @return グループインスタンス
	 */
	public PosixGroupWithList getGroupWithList(String name, List<String> users) {
