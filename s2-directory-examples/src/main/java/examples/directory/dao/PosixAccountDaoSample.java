...
public class PosixAccountDaoSample {
	private static final String PATH = "examples/directory/dao/PosixAccountDao.dicon";
	...
	public static void authenticateByUser1() {
		...
		PosixAccountDao dao = (PosixAccountDao)container
				.getComponent(PosixAccountDao.class);
		if (dao.authenticateByUser(property)) {
			System.out.println("ok");
		} else {
			System.out.println("false");
		}
	...
	public static void getUserByUidWithUser() {
		...
		PosixAccountDao dao = (PosixAccountDao)container
				.getComponent(PosixAccountDao.class);
		// user1を取得します。
		PosixAccount account = dao.getUserByUidWithUser(property, "user1");
		// 表示します。
		System.out.println("DEBUG: " + account);
		// user2を取得します。
		account = dao.getUserByUidWithUser(property, "user2");
		// 表示します。
		System.out.println("DEBUG: " + account);
	}
	...
	public static void selectAndSelect() {
		...
		// account.setDn(null);
		PosixAccount account2 = new PosixAccount();
	...
	public static void selectAndUpdateUserWithUser() {
		...
		int amount = dao.updateWithUser(property, account);
		property.setPassword("test");
		account = dao.getUserByUid("user2");
		account.setUserPassword("user2pass");
		amount += dao.updateWithUser(property, account);
		System.out.println("UPDATE AMOUNT: " + amount);
	}
